...
const buildGraph = (map, [startPosition, keys, doors]) => {
  ...
  // add cost between a door and its key
  matched.forEach(([_,position], key) => {
    const  [x, y, distance, foundItems] = search(map, position, key.toUpperCase());
    graph = addEdgeToGraph(graph, key, key.toUpperCase(), distance);
  });

  const directDoorsToTheOnekey = doorsOnPath(foundItems);
  directDoorsToTheOnekey.forEach(door => {
    const [dx, dy, dcount, dFoundItems] = search(map, [x, y], door);
    const [kx, ky, kcount, kFoundItems] = search(map, [dx, dy], door.toLowerCase());
    graph = addEdgeToGraph(graph, door.toLowerCase(), door, kcount);
  });
...
const test = () => {
  ...
  let graph = buildGraph(map, [startPosition, keys, doors]);
